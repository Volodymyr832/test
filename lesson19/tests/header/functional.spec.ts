...
test.describe('Header Functional Tests', () => {
  ...
  test('should open menu on hamburger click', async () => {
    await header.openMenu();
    expect(await header.isExpandedMenuVisible()).toBe(true);
  });

  test('should show submenu on second menu item hover', async () => {
    await header.openMenu();
    const isShown = await header.hoverOnSecondMenuItem();
    expect(isShown).toBe(true);
  });

  test('should allow typing in search input, show and clear with clear button', async () => {
