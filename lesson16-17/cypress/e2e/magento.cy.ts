...
describe('Magento Search Tests', () => {
    ...
    it('Search for "jacket"', () => {
        cy.get('input#search', { timeout: 10000 }).should('be.visible').type('jacket{enter}');
    ...
    it('Filter by category', () => {
        ...
        cy.contains('.filter-options-title', 'Category', { timeout: 10000 }).should('be.visible').click();

        cy.get('.filter-options-content .item a', { timeout: 10000 }).first().should('be.visible').click();
