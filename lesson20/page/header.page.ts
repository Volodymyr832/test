...
export class HeaderPage {
  ...
  constructor(page: Page) {
    ...
    this.hamburgerButton = this.hamburgerButton =
      this.shadowHeader.locator(':shadow=#trigger-button');
    this.expandedMenu = this.shadowHeader.locator('nav ul li');
    this.searchInput = this.shadowHeader.locator('input[type="text"]');
    this.clearButton = this.shadowHeader.locator('igds-icon.search__clear-icon[role="button"]');
    this.bottomMenuToggle = this.shadowHeader.locator('button[aria-label="Open bottom menu"]');
  ...
  async openMenu(): Promise<void> {
    const buttonHandle = await this.frameLocator
      .locator('igds-header')
      .evaluateHandle((el) => el.shadowRoot?.querySelector('#trigger-button') as HTMLElement);
  ...
  async isExpandedMenuVisible(): Promise<boolean> {
    const menuHandle = await this.frameLocator
      .locator('igds-header')
      .evaluateHandle((el) => el.shadowRoot?.querySelector('.header__hamburger-button'));
  ...
  async hoverOnSecondMenuItem(): Promise<boolean> {
    const menuItemHandle = await this.page
      .frameLocator('iframe')
      .locator('igds-header')
      .evaluateHandle((header) => header.shadowRoot?.querySelectorAll('igds-menu-item')[1]);
    ...
    const submenuVisible = await this.page
      ...
      .evaluate((header) => {
        ...
        const submenu = item?.shadowRoot?.querySelector('div.menu-item__submenu');
